...
def apply_theme():
    ...
    st.markdown("""
    <style>
      .st-key-task_navigation {
        width: 340px; max-width: 100%; align-self: flex-start;
        margin: 0 auto 8px 0; }
      .st-key-task_navigation label p {
        color: #475569; font-size: 13px; font-weight: 600; }
      .st-key-task_navigation [data-baseweb="select"] > div {
        background-color: #f8fafc; border-color: #cbd5e1;
        border-radius: 8px; min-height: 40px; font-size: 14px; }
      .st-key-task_navigation [data-baseweb="select"] > div:hover {
        border-color: #103673; }
      .st-key-task_navigation [data-baseweb="select"]:focus-within {
        outline: 2px solid #2563eb; outline-offset: 2px; border-radius: 8px; }

      .stMain h3, .stMain h4, .stMain h5 { color: #103673; }
      ...
      .stMain [data-testid="stMetric"] { background: #f8fafc; border: 1px solid #e2e8f0;
        border-radius: 8px; padding: 12px; }
      .stMain [data-testid="stMetricValue"] { font-size: 1.25rem; }
    </style>
...
def render_metrics(metrics):
    ...
    items = [
        ("Silhouette", format_score(metrics["Silhouette"])),
        ("Davies-Bouldin", format_score(metrics["Davies-Bouldin"])),
        ("Calinski-Harabasz", format_score(metrics["Calinski-Harabasz"], 1)),
        ("Thời gian (giây)", format_score(metrics["Time"])),
    ]
